...
public class QuerydslRepositorySupportTests {
	...
	public void shouldAllowDbRefAgainstIdProperty() {
		...
		User p2 = new User();
		p2.setUsername("bart@simpson.com");
		operations.save(p2);

		person.setCoworker(p2);
		operations.save(person);

		QPerson p = QPerson.person;

		SpringDataMongodbQuery<Person> queryUsingIdField = repoSupport.from(p).where(p.coworker.id.eq(p2.getId()));
		SpringDataMongodbQuery<Person> queryUsingRefObject = repoSupport.from(p).where(p.coworker.eq(p2));
